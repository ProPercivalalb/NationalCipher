...
public interface ICipher<K> {
    ...
    /**
     * Iterates thought all the ciphers keys, the key object K is not
     * necessarily immutable so if access is required after a copy is needed
     */
    void iterateKeys(Consumer<K> consumer);
    
    /**
     * Used for simulated annealing, returns a key that is similar to
     * the given key
     */
