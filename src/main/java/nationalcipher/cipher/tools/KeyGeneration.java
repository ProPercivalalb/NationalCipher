	        ...
	        while(rowOpts.size() > 0) {
	            ...
	            while(true) {
	               ...
	               // System.out.println("Try "  + test);
	                ...
	                if(notAllowed) {
	                //if(slotsLeft - 1 < possiblities && (rowOpts.stream().map(Pair::getRight).filter(p -> p != pair && p.contains(test)).anyMatch(p -> p.size() - 1 < slotsLeft - 1) || rowOpts.stream().map(Pair::getRight).anyMatch(p -> p != pair && p.isEmpty()))) {
