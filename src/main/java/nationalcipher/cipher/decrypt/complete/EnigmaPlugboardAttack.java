    ...
    public void attemptAttack(String text, DecryptionMethod method, IApplication app) {
        ...
        if (method == DecryptionMethod.BRUTE_FORCE) {
            ...
            for (int option = 0; option < task.squeezeSecond.size(); option++) {
                ...
                /**
                 * int plugboardIndex = 0; int[][] plugboard = new int[13][2]; int[]
                 * possiblePlugBoard = new int[26]; for(int i = 0; i < 26; i++)
                 * possiblePlugBoard[i] = i; while(true) {
                 * 
                 * task.plainText = Enigma.decode(task.cipherText, task.plainText,
                 * trial.machine, trial.copyIndicator(), trial.ring, trial.rotors,
                 * trial.reflector, plugboard);
                 * 
                 * Solution bestSolution = new Solution(task.plainText,
                 * app.getLanguage().getTrigramData()).bakeSolution(); int bestPlug1 = 0; int
                 * bestPlug2 = 1; byte[] testText = Arrays.copyOf(task.plainText,
                 * task.plainText.length); boolean foundFinalPlug = true; for(int i1 = 0; i1 <
                 * possiblePlugBoard.length - 1; i1++) { for(int i2 = i1 + 1; i2 <
                 * possiblePlugBoard.length; i2++) { if(i1 == i2) { app.out().println("Same
                 * letter: "); continue; } plugboard[plugboardIndex][0] = possiblePlugBoard[i1];
                 * plugboard[plugboardIndex][1] = possiblePlugBoard[i2]; testText =
                 * Enigma.decode(task.cipherText, testText, trial.machine,
                 * trial.copyIndicator(), trial.ring, trial.rotors, trial.reflector, plugboard);
                 * Solution lastSolution = new Solution(testText,
                 * app.getLanguage().getTrigramData());
                 * 
                 * if(lastSolution.isResultBetter(bestSolution)) { bestSolution = lastSolution;
                 * bestSolution.setKeyString("%c%c", plugboard[plugboardIndex][0],
                 * plugboard[plugboardIndex][1]); bestPlug1 = plugboard[plugboardIndex][0];
                 * bestPlug2 = plugboard[plugboardIndex][1]; bestSolution.bakeSolution();
                 * foundFinalPlug = false; } } }
                 * 
                 * plugboard[plugboardIndex][0] = bestPlug1; plugboard[plugboardIndex][1] =
                 * bestPlug2;
                 * 
                 * if(foundFinalPlug) { plugboardIndex++; char[] plugs = new
                 * char[Math.max(plugboardIndex * 3 - 1, 0)]; Arrays.fill(plugs, ' '); for(int p
                 * = 0; p < plugboardIndex; p++) { plugs[p * 3] = (char)(plugboard[p][0] + 'A');
                 * plugs[p * 3 + 1] = (char)(plugboard[p][1] + 'A'); }
                 * bestSolution.setKeyString("%s, Plugs:[%s]", trial.toKeyString(), new
                 * String(plugs)); app.out().println("%s", bestSolution);
                 * if(bestSolution.isResultBetter(task.bestSolution)) task.bestSolution =
                 * bestSolution; break; }
                 * 
                 * 
                 * int[] possiblePlugBoardNext = new int[possiblePlugBoard.length - 2]; int
                 * currentIndex = 0; for(int i = 0; i < possiblePlugBoard.length; i++)
                 ...
                 * trial.copyIndicator(); int[] ring = new int[] {trial.ring[0], trial.ring[1],
                 * trial.ring[2]};
                 * 
                 * indicator[1] = (indicator[1] + s2) % 26; indicator[2] = (indicator[2] + s3) %
                 * 26;
                 * 
                 * ring[1] = (ring[1] + s2) % 26; ring[2] = (ring[2] + s3) % 26;
                 * 
                 * task.plainText = Enigma.decode(task.cipherText, task.plainText,
                 * trial.machine, indicator, ring, trial.rotors, -1, trial.reflector,
                 * plugboard); Solution lastSolution = new Solution(task.plainText,
                 * app.getLanguage());
                 * 
                 * if(lastSolution.isResultBetter(bestSolution2)) { bestSolution2 =
                 * lastSolution; bestSolution2.setKeyString("%s, Plugs:[%s]",
                 * trial.toKeyString(), str); bestSolution2.bakeSolution(); } } }
                 * app.out().println("%s", bestSolution2);
                 * if(bestSolution2.isResultBetter(task.bestSolution)) task.bestSolution =
                 * bestSolution2;
                 **/
