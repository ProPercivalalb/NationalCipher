...
public class NihilistSubstitutionCipher extends BiKeyCipher<String, String> {
    ...
    public NihilistSubstitutionCipher() {
        super(FullStringKeyType.builder().setAlphabet(KeyGeneration.ALL_25_CHARS),
                VariableStringKeyType.builder().setAlphabet(KeyGeneration.ALL_25_CHARS).setRange(2, 15));
    }
    ...
    public char[] decodeEfficently(CharSequence cipherText, @Nullable char[] plainText, BiKey<String, String> key) {
        ...
        for(int i = 0; i < cipherText.length() / 2; i++) {
            ...
            //TODO if(row * 5 + column - 1 >= key.getFirstKey().length() || row * 5 + column - 1 < 0) return StringTransformer.repeat("Z", plainText.length).toCharArray();
