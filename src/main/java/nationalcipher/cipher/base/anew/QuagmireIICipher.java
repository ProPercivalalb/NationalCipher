...
public class QuagmireIICipher extends TriKeyCipher<String, String, Character, FullStringKeyType.Builder, VariableStringKeyType.Builder, ObjectKeyType.Builder<Character>> {
    ...
    public CharSequence encode(CharSequence plainText, TriKey<String, String, Character> key, IFormat format) {
        return Quagmire.encode(plainText, Alphabet.getUpperCase(), key.getFirstKey(), key.getSecondKey(), key.getThirdKey());
    }

    @Override
    public char[] decodeEfficently(CharSequence cipherText, @Nullable char[] plainText, TriKey<String, String, Character> key) {
        return Quagmire.decode(cipherText, plainText, Alphabet.getUpperCase(), key.getFirstKey(), key.getSecondKey(), key.getThirdKey());
    }
}
