...
public class Challenge7Attack implements IRandEncrypter {
	...
	public static byte[] decode(char[] cipherText, byte[] plainText, String key, VigenereType type) {
		...
		for(int index = 0; index < cipherText.length; index++) {
			if(cipherText[index] == ' ') {
				...
				plainText[index] = type.decode((byte)cipherText[index], (byte)key.charAt(count++ % period));
		}
		
		return ArrayUtil.plainText;
	}
